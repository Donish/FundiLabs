...
int symbolandminus_in_int(char* number)
{
...
int check_cde(int amount, char* number, int size)
{
    if(amount != 4){
        printf("Enter the number with this flag!\n");
        return -1;
    }
    if(symbolandminus_in_int(number)){
        printf("There is a symbol in your number!\n");
        return -1;
    }
    int idx = strtoint(number);
    if(idx > size - 1){
...
int main(int argc, char* argv[])
{
    ...
    if(strcmp(argv[2], "-a") == 0){
        ...
        if(argc != 3){
            printf("Enter only the file name and the flag!\n");
            return 0;
        }
        for(int i = 0; i < size; i++){
            if(i % 2 == 0){
        ...
        int idx = check_cde(argc, argv[3], size);
        if(idx == -1){
            return 0;
        }
        new_size = 1;
