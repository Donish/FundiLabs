...
double strtodouble(char* number)
{
    ...
    double res = 0;
    int sign = 0;
    if(number[0] == 45){ //проверка знака числа
        sign = -1;
    } else{
        sign = 1;
    }
    for(int i = 0; number[i] != '\0'; i++, power++){ //перевод из строки в дабл
        if(number[i] == 44 || number[i] == 46){
            power = 0;
            continue;
        }
        if(i == 0 && number[i] == 45){
...
int symbol_in_double(char* number)
{
    for(int i = 0; number[i] != '\0'; i++){
        if(number[i] == 45 && i == 0){
            continue;
        }
        if((number[i] == 44 || number[i] == 46) && i != 0){
            continue;
        }
    ...
    if(strcmp(argv[1], "-q") == 0 || strcmp(argv[1], "/q") == 0){
        if(argc != 5){
            ...
            if(fabs(a) < eps && fabs(b) > eps && fabs(c) > eps){
                ...
                printf("No solutions\n");
                return 0;
            } else if((fabs(a) > eps && fabs(b) < eps && fabs(c) < eps) || (fabs(a) < eps && fabs(b) > eps && fabs(c) < eps)){
                double x = 0;
                printf("One solution:\n%f\n", x);
                return 0;
            }
        ...
        if(argc != 4){
            ...
            if(symbol_in_int(argv[2]) || symbol_in_int(argv[3])){
